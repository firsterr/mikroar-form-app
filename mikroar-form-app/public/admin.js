/* MikroAR – Admin Form Builder (desc destekli) */
...
const toast = (m, type='')=>{
  ...
  t.style.borderColor = type==='err' ? '#ef4444' : type==='ok' ? '#22c55e' : '#e5e7eb';
...
function setForm(form){
  ...
  // açıklama: önce top-level, yoksa schema.description
  els.desc.value   = (form.description || form.schema?.description || '');
...
async function save(){
  ...
  const body = {
    ...
    // açıklamayı schema.description içine yazıyoruz
    schema: { description: els.desc.value.trim(), questions }
  };
  try{
...
els.btnNew.onclick = ()=>{ els.title.value=''; els.active.value='true'; els.desc.value=''; questions=[]; render(); };
